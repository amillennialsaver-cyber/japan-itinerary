...
function MapEmbed({ day }: { day: Day }) {
  const q = encodeURIComponent(`${day.city}, Japan`);
  return (
    <div style={{ borderRadius: 4, overflow: "hidden", border: "1px solid var(--border)", height: 260 }}>
      <iframe
        title={`Map of ${day.city}`}
        width="100%"
        height="260"
...
function WeekBadge({ week }: { week: number }) {
  const labels = ["", "Week 1 — Tokyo", "Week 2 — Kansai", "Week 3 — West & Race"];
  return (
    <div style={{ padding: "0.3rem 0.75rem", background: "var(--navy-light)", borderRadius: 2, display: "inline-block", marginBottom: "0.5rem" }}>
      <span style={{ fontSize: "0.6rem", fontFamily: "'DM Mono', monospace", color: "var(--navy)", letterSpacing: "0.1em" }}>
...
export default function Home() {
  ...
  const regionColor = REGION_COLORS[day.region] ?? "#1A2744";
    ...
    <div style={{ minHeight: "100vh", background: "var(--white)" }}>
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "0.75rem" }}>
          <span style={{ fontFamily: "'Shippori Mincho', serif", fontSize: "1.3rem", color: "rgba(255,255,255,0.6)" }}>日本</span>
          <span style={{ fontSize: "1.1rem", fontWeight: 500, letterSpacing: "0.05em" }}>JAPAN 2027</span>
          <span style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.5)", marginLeft: "0.5rem" }}>3 WEEKS FOR TWO · WITH F1 AT SUZUKA</span>
        </div>
        <div style={{ marginLeft: "auto", display: "flex", gap: "0.5rem" }}>
          {(["itinerary", "info"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              padding: "0.4rem 1rem",
              background: activeTab === tab ? "var(--red)" : "transparent",
              border: "1px solid rgba(255,255,255,0.2)", color: "white",
              fontSize: "0.75rem", letterSpacing: "0.08em", cursor: "pointer",
              textTransform: "uppercase", fontFamily: "'DM Mono', monospace", borderRadius: 2,
            }}>
              {tab === "itinerary" ? "📅 Itinerary" : "ℹ️ Useful Info"}
            </button>
          ))}
        </div>
      </header>
      ...
      {activeTab === "info" ? (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "3rem 2rem" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "3rem", gap: "2rem", flexWrap: "wrap" }}>
            <div>
              <h2 className="japanese" style={{ fontSize: "2rem", fontWeight: 600, marginBottom: "0.5rem", color: "var(--red)" }}>Essential Travel Info</h2>
              <p style={{ color: "var(--muted)", fontSize: "0.9rem" }}>Everything you need to know for an incredible 3-week Japan trip.</p>
            </div>
            <div style={{ textAlign: "center", flexShrink: 0 }}>
              <img src="/couple.png" alt="Tills and Harry's Japan Itinerary 2027" style={{ width: 160, height: 160, objectFit: "cover", borderRadius: 8, border: "2px solid var(--border)", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }} />
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: "1.5rem" }}>
            {usefulInfo.map(section => (
              <div key={section.category} style={{ border: "1px solid var(--border)", borderRadius: 4, padding: "1.5rem", background: "var(--off-white)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem" }}>
                  <span style={{ fontSize: "1.5rem" }}>{section.icon}</span>
                  <h3 style={{ fontSize: "1rem", fontWeight: 600, color: "var(--navy)" }}>{section.category}</h3>
                </div>
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                  {section.items.map((item, i) => (
                    <li key={i} style={{ display: "flex", gap: "0.6rem", fontSize: "0.85rem", lineHeight: 1.55, color: "var(--ink)" }}>
        ...
        <div style={{ display: "flex" }}>
          <aside style={{
            width: sidebarOpen ? 270 : 0, minWidth: sidebarOpen ? 270 : 0,
            overflow: "hidden", transition: "all 0.3s ease",
            borderRight: "1px solid var(--border)",
            height: "calc(100vh - 63px)", position: "sticky", top: 63,
            overflowY: "auto", background: "var(--off-white)", flexShrink: 0,
          }}>
            <div style={{ width: 270 }}>
              <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid var(--border)", background: "var(--navy)" }}>
                <p style={{ fontSize: "0.6rem", fontFamily: "'DM Mono', monospace", color: "rgba(255,255,255,0.5)", letterSpacing: "0.1em", marginBottom: "0.4rem" }}>YOUR ROUTE</p>
                <p style={{ fontSize: "0.8rem", color: "white", lineHeight: 1.6 }}>
                  Tokyo → Nikko → Hakone → Kyoto → Nara → Suzuka 🏎️ → Osaka → Hiroshima → Miyajima
                </p>
              </div>
              {[1,2,3].map(week => (
                <div key={week}>
                  <div style={{ padding: "0.6rem 1.25rem 0.3rem", background: "rgba(26,39,68,0.05)", borderBottom: "1px solid var(--border)" }}>
                    <span style={{ fontSize: "0.6rem", fontFamily: "'DM Mono', monospace", color: "var(--muted)", letterSpacing: "0.1em" }}>
                      {week === 1 ? "WEEK 1 — TOKYO" : week === 2 ? "WEEK 2 — KANSAI" : "WEEK 3 — WEST & RACE"}
                    </span>
                  </div>
                  {itinerary.filter(d => Math.ceil(d.day / 7) === week).map(d => (
                    <button key={d.day} onClick={() => setActiveDay(d.day)} style={{
                      width: "100%", textAlign: "left", padding: "0.6rem 1.25rem",
                      background: activeDay === d.day ? "white" : "transparent",
                      border: "none",
                      borderLeft: activeDay === d.day ? `3px solid ${REGION_COLORS[d.region] ?? "#9B1B30"}` : "3px solid transparent",
                      cursor: "pointer", display: "flex", alignItems: "center", gap: "0.6rem",
                      borderBottom: "1px solid rgba(0,0,0,0.04)", transition: "all 0.15s",
                    }}>
                      <span style={{ fontSize: "1.1rem", flexShrink: 0 }}>{d.emoji}</span>
                      <span style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ display: "block", fontSize: "0.6rem", fontFamily: "'DM Mono', monospace", color: "var(--muted)" }}>DAY {d.day} · {d.date}</span>
                        <span style={{ display: "block", fontSize: "0.82rem", fontWeight: activeDay === d.day ? 600 : 400, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.city}</span>
                      </span>
                      <span style={{
                        fontSize: "0.55rem", padding: "0.15rem 0.4rem",
                        background: (REGION_COLORS[d.region] ?? "#9B1B30") + "22",
                        color: REGION_COLORS[d.region] ?? "#9B1B30",
          ...
          <button onClick={() => setSidebarOpen(o => !o)} style={{
            position: "fixed", left: sidebarOpen ? 270 : 0, top: "50%",
            transform: "translateY(-50%)", background: "var(--navy)", color: "white",
            border: "none", padding: "0.5rem 0.3rem", cursor: "pointer", zIndex: 50,
            fontSize: "0.7rem", borderRadius: "0 4px 4px 0", transition: "left 0.3s ease",
          }}>{sidebarOpen ? "◂" : "▸"}</button>

          <main style={{ flex: 1, padding: "2.5rem 3rem", maxWidth: 820, overflowY: "auto" }}>
            <div key={activeDay} className="fade-up">
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "2rem" }}>
                {day.highlights.map(h => (
                  <span key={h} style={{ padding: "0.3rem 0.75rem", background: "var(--off-white)", border: "1px solid var(--border)", borderRadius: 20, fontSize: "0.78rem", color: "var(--ink)" }}>✦ {h}</span>
                ))}
              </div>

              <div style={{ marginBottom: "2rem" }}><MapEmbed day={day} /></div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0", marginBottom: "2rem" }}>
                {[
                  ...
                  <div key={label} style={{ display: "grid", gridTemplateColumns: "90px 1fr", gap: "1.25rem", padding: "1.5rem 0", borderBottom: i < 2 ? "1px solid var(--border)" : "none" }}>
                    <div style={{ paddingTop: "0.15rem" }}>
                      <div style={{ fontSize: "1.3rem", marginBottom: "0.25rem" }}>{icon}</div>
                      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--muted)", letterSpacing: "0.1em" }}>{label.toUpperCase()}</span>
                    </div>
                    <div>
                      <h3 style={{ fontSize: "1rem", fontWeight: 600, color: "var(--navy)", marginBottom: "0.4rem" }}>{data.title}</h3>
                      <p style={{ fontSize: "0.9rem", lineHeight: 1.75, color: "#374151" }}>{data.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "2.5rem" }}>
                <div style={{ padding: "1.25rem", background: "#FFF8F0", border: "1px solid #F5D8B0", borderRadius: 4 }}>
                  <p style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--gold)", letterSpacing: "0.12em", marginBottom: "0.4rem" }}>🍜 WHERE TO EAT</p>
                  <p style={{ fontSize: "0.85rem", lineHeight: 1.6, color: "var(--ink)" }}>{day.eat}</p>
                </div>
                <div style={{ padding: "1.25rem", background: "#F0F4FF", border: "1px solid #C7D3F5", borderRadius: 4 }}>
                  <p style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--navy)", letterSpacing: "0.12em", marginBottom: "0.4rem" }}>💡 INSIDER TIP</p>
                  <p style={{ fontSize: "0.85rem", lineHeight: 1.6, color: "var(--ink)", fontStyle: "italic" }}>{day.tip}</p>
                </div>
              </div>

              {day.transport && (
                <div style={{ padding: "0.9rem 1.25rem", background: "var(--off-white)", border: "1px solid var(--border)", borderRadius: 4, marginBottom: "2rem", display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                  <span style={{ fontSize: "1.1rem", flexShrink: 0 }}>🚆</span>
                  <div>
                    <p style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.6rem", color: "var(--muted)", letterSpacing: "0.1em", marginBottom: "0.2rem" }}>GETTING THERE</p>
                    <p style={{ fontSize: "0.85rem", color: "var(--ink)" }}>{day.transport}</p>
                  </div>
                </div>
              )}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", paddingTop: "1.5rem", borderTop: "1px solid var(--border)" }}>
                {activeDay > 1 ? (
                  <button onClick={() => setActiveDay(d => d - 1)} style={{ padding: "0.65rem 1.25rem", background: "white", border: "1px solid var(--border)", cursor: "pointer", fontSize: "0.85rem", color: "var(--muted)", borderRadius: 3, fontFamily: "'DM Mono', monospace" }}>← Day {activeDay - 1}</button>
                ) : <div />}
                {activeDay < 21 && (
                  <button onClick={() => setActiveDay(d => d + 1)} style={{ padding: "0.65rem 1.5rem", background: "var(--navy)", border: "none", cursor: "pointer", fontSize: "0.85rem", color: "white", borderRadius: 3, fontFamily: "'DM Mono', monospace" }}>Day {activeDay + 1} →</button>
                )}
              </div>
            </div>
          </main>
